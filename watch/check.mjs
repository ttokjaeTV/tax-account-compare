...
lines.push('---');
lines.push('키움·메리츠·KB(PDF 공시)와 절세계좌 이벤트 페이지는 자동 감시 대상이 아닙니다. 이벤트는 회차마다 URL이 바뀌므로 만료 캘린더로 관리합니다. 분기 갱신 때 직접 확인하세요.');
...
if (process.env.GITHUB_OUTPUT) {
  ...
  const title = (t1.length ? '🚨 ' : (due.length ? '📅 ' : '')) + `절세계좌 감시 — ${parts.join(' · ')} (${today})`;
